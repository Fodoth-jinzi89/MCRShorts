...
public class GoldSlabRenderer implements BlockEntityRenderer<GoldSlabBlockEntity> {
	private final CustomHierarchicalModel model;
	private final ResourceLocation texture;
	private final LivingEntityRenderState renderState;

	GoldSlabRenderer(BlockEntityRendererProvider.Context context) {
		this.model = new CustomHierarchicalModel(context.bakeLayer(Modelgold_slab1.LAYER_LOCATION));
		this.texture = ResourceLocation.parse("mcrshorts:textures/block/gold_slab.png");
		this.renderState = new LivingEntityRenderState();
	}

	private void updateRenderState(GoldSlabBlockEntity blockEntity, float partialTick) {
	...
	@Override
	public void render(GoldSlabBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource renderer, int light, int overlayLight) {
		updateRenderState(blockEntity, partialTick);
		poseStack.pushPose();
		poseStack.scale(-1, -1, 1);
		poseStack.translate(-0.5, -0.5, 0.5);
		BlockState state = blockEntity.getBlockState();
		Direction facing = state.getValue(GoldSlabBlock.FACING);
		switch (facing) {
			case NORTH -> {
			}
	...
	public static void registerBlockEntityRenderers(EntityRenderersEvent.RegisterRenderers event) {
		event.registerBlockEntityRenderer(McrshortsModBlockEntities.GOLD_SLAB.get(), GoldSlabRenderer::new);
	}

	private static final class CustomHierarchicalModel extends Modelgold_slab1 {
		public CustomHierarchicalModel(ModelPart root) {
			super(root);
		}

		public void setupBlockEntityAnim(GoldSlabBlockEntity blockEntity, LivingEntityRenderState state) {
